...
def main(args=None):
    ...
    with open('/mnt/nova_ssd/workspaces/ai_reconstruction_ws/our_dataset/depth.txt', 'w') as file:
        for depth_file in depth_files:
            filename = depth_file
            lst = filename.split("/")
            depth_name = os.path.join(lst[5], lst[6])
            file.write(f'{depth_name}\n')
    ...
    with open('/mnt/nova_ssd/workspaces/ai_reconstruction_ws/our_dataset/rgb.txt', 'w') as file:
        for rgb_file in rgb_files:
            filename = rgb_file
            lst = filename.split("/")
            rgb_name = os.path.join(lst[5], lst[6])
            file.write(f'{rgb_name}\n')

    #####################################################################################################
    # Write the calibration parameters to a YAML file

    config_file = os.path.join("our_dataset", "calibration.yaml")
    config = {
        "inherit_from": "our_config.yaml",
        "Dataset": {
            "dataset_path": "our_dataset"
        },
        "Calibration": {
            "fx": 908.7236938476562,
            "fy": 907.1197509765625,
            "cx": 639.1090087890625,
            "cy": 381.8516845703125,
            "k1": 0,
            "k2": 0,
            "p1": 0,
            "p2": 0,
